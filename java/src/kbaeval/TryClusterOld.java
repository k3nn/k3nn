 ...
 * @author jeroen
 */
public class TryClusterOld {

    public static final Log log = new Log(TryClusterOld.class);
    Stream<NodeS> stream = new Stream();
    DefaultTokenizer tokenizer = Stream.getUnstemmedTokenizer();
    Cluster cluster;

    public TryClusterOld(String yesterday, String today, int clusterid) {
        Cluster<NodeS> end = readResults(today, clusterid);
        stream = new Stream();
        Cluster<NodeS> start = readResults(yesterday, clusterid);
...
//            log.info("%d %s", u.getKey(), u.getValue());
    ...
    public static void main(String[] args) {
        ArgsParser conf = new ArgsParser(args, "-b yesterday -t today -c clusterid");
        TryClusterOld relevance = new TryClusterOld(conf.get("yesterday"), conf.get("today"), conf.getInt("clusterid", 0));
    }
}
